...
워프가 락스텝으로 움직인다는 사실은 조건문 앞에서 비용으로 바뀐다.
...
하드웨어는 스레드마다 다른 명령어를 동시에 실행할 방법이 없다. 그래서 워프는 참 경로(A)를 먼저 실행하며 거짓인 16개 스레드를 놀리고, 그다음 거짓 경로(B)를 실행하며 참인 16개 스레드를 놀린다. 이를 워프 분기(Warp Divergence)라 부른다. 원래 1번이면 끝났을 실행이 2번(A + B)으로 늘어나고, 워프가 N갈래로 갈리면 N배로 늘어난다 — 정확히는 워프가 순차 실행해야 하는 경로 수만큼 사이클이 늘어난다.

이 비용은 코드만 봐서는 눈에 띄지 않는다는 점이 위험하다. 그래서 GPU 코드를 짤 때는 연속된 32개 스레드(워프 하나)가 같은 분기를 타도록 조건을 배치하는 것이 실질적인 성능 튜닝 포인트가 된다.
...
SM 하나에는 워프 스케줄러가 여러 개 있고, 동시에 수십 개의 워프를 대기 상태로 들고 있을 수 있다. 워프 A가 글로벌 메모리 접근으로 수백 사이클을 기다려야 하면, SM은 그 시간에 놀지 않고 이미 준비된 워프 B, C를 실행 유닛에 채워 넣는다. CPU가 캐시 미스 한 번에 파이프라인이 멈추는 것과 달리, GPU는 대기 중인 워프를 대량으로 쌓아두고 빠르게 전환하는 방식으로 지연시간을 처리량 뒤로 숨긴다(latency hiding). SM에 동시에 상주할 수 있는 워프 수(occupancy)가 높을수록 이 은닉 효과가 커진다. 결국 GPU가 처리량을 극대화하는 방식은 "지연시간을 없애는 것"이 아니라 "지연시간이 보이지 않을 만큼 다른 일을 채워 넣는 것"이다.
...
지연 시간 수치는 세대·제조사마다 다르지만 자릿수 차이는 일관된다. 글로벌 메모리 접근 한 번이 레지스터 접근 수백 배에 달하는 사이클을 잡아먹는다는 뜻이다. 행렬곱처럼 같은 데이터를 여러 스레드가 반복해서 읽는 연산에서, 글로벌 메모리 접근을 공유 메모리 접근으로 바꾸면 지연 시간이 400~800사이클에서 20~30사이클 수준으로 줄어든다. GPU 코드 최적화의 상당 부분이 결국 "얼마나 많은 접근을 이 표의 위쪽 계층으로 끌어올리느냐"의 문제다.
