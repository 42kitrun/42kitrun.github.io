...
async function renderGraph(graph: HTMLElement, fullSlug: FullSlug) {
  ...
  for (const [source, details] of data.entries()) {
    ...
    if (showTags) {
      ...
      const localProjects = (details.relatedProjects ?? []).map((project) =>
        simplifySlug(("projects/" + project) as FullSlug),
      );

      projects.push(
        ...localProjects.filter((project) => !projects.includes(project)),
      );

      for (const project of localProjects) {
        links.push({ source: source, target: project });
      }
    }
  }
  ...
  if (depth >= 0) {
    ...
    if (showTags) projects.forEach((project) => neighbourhood.add(project));
  }
  ...
  const nodes = [...neighbourhood].map((url) => {
    ...
    return {
      ...
      relatedProjects: data.get(url)?.relatedProjects ?? [],
    };
  });
  ...
  for (const n of graphData.nodes) {
    ...
    const isProjectNode = nodeId.startsWith("projects/");
  ...
  if (enableDrag) {
      ...
      drag<HTMLCanvasElement, NodeData | undefined>()
        ...
        .on("end", function dragended(event) {
          ...
          if (Date.now() - dragStartTime < 500) {
            ...
            if (node.id.startsWith("projects/")) return;
    ...
    for (const node of nodeRenderData) {
      node.gfx.on("click", () => {
        if (node.simulationData.id.startsWith("projects/")) return;
