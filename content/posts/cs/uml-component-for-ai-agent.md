---
title: "컴포넌트 다이어그램: 무엇으로 나누어 개발할 것인가"
date: 2026-06-18
updated: 2026-06-18
tags:
  - uml
...
summary: AI Agent와 함께 시스템이나 서비스를 개발할 때 컴포넌트 다이어그램으로 책임 단위, 인터페이스, 의존성을 나누어 자연어 지시의 모호함을 줄이는 방법을 탐구한다.
...
**그 책임들은 어떤 구현 단위로 나뉘어야 하는가?**

컴포넌트 다이어그램은 이 질문을 탐구하는 도구다. 시퀀스 다이어그램이 실행 중 오가는 메시지를 보여준다면, 컴포넌트 다이어그램은 그 메시지를 담당할 구성 요소와 의존성을 보여준다.

여기서 중요한 것은 AI Agent 시스템이나 서비스를 만드는 방법이 아니다. 이 글의 초점은 **AI Agent를 활용해 개발할 때, 자연어 요구사항을 어떤 책임 단위로 나누면 지시와 리뷰가 명확해지는가**에 있다.
...
![컴포넌트 다이어그램으로 AI Agent 개발 작업 경계 나누기|697](/assets/posts/cs/uml-component-for-ai-agent/component_diagram_for_ai_agent.svg)
...
컴포넌트 다이어그램은 AI Agent와 함께 시스템이나 서비스를 개발할 때 "무엇으로 나누어 개발할 것인가"를 탐구하는 도구다.
...
이것이 AI Agent 활용 개발에서 컴포넌트 다이어그램이 해결하는 문제다. AI Agent 시스템을 만드는 것이 아니라, AI Agent와 함께 개발할 시스템을 더 명확히 나누어 설명하는 것이다.
...
- **현재**: 5편 - 컴포넌트 다이어그램 (무엇으로 나누어 개발할 것인가)
