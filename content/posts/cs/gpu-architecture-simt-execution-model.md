...
- GPU는 스레드마다 디코드 회로를 두는 대신 32개씩 워프로 묶어 명령어 디코더 하나를 공유시킨다. SIMT 모델은 이 방식으로 코어 수천 개를 낮은 관리 비용으로 굴린다.
- 워프는 락스텝으로 움직이기 때문에 조건문에서 경로가 갈리면(Warp Divergence) 경로 수만큼 실행이 순차적으로 늘어난다. 워프 내부가 같은 분기를 타도록 코드를 짜는 이유가 여기에 있다.
- 실행 단위는 스레드 → 워프 → 스레드 블록 → 그리드로 쌓이고, SM이 이를 물리적으로 스케줄링한다.
- SM은 워프를 대량으로 대기시켜 두고, 한 워프가 메모리를 기다리는 동안 다른 워프로 실행 유닛을 채운다(latency hiding).
