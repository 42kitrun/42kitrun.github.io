...
JVM 서버가 실행되는 환경의 계층 구조다. 아래에서 위로 쌓인다.

```
┌──────────────────────────────────────────┐
│               Hardware                   │
│   CPU / RAM / NIC (네트워크 카드) / 디스크│
└────────────────────┬─────────────────────┘
                     │
┌────────────────────▼─────────────────────┐
│             OS Kernel                    │
│  Linux (서버 대부분) / Windows Server    │
│                                          │
│  역할: TCP/IP 스택 처리, 소켓 관리,       │
│        스레드 스케줄링, 메모리 페이징,    │
│        NIC 드라이버 → 패킷 수신          │
└────────────────────┬─────────────────────┘
                     │
...
│  (소켓 read/write, 파일 I/O 등)        │
└──────────────────────────────────────────┘
```

JVM도 OS 입장에서 하나의 프로세스다. 차이점은 내부에서 다수의 스레드를 운영하고, GC가 메모리를 관리하며, JIT 컴파일로 바이트코드를 동적으로 최적화한다는 것이다.
...
│  └──────────────────────────────────────────────┘   │
│                                                     │
│  → [[javascript/how-javascript-runs|JS JIT]]와 비슷하게         │
│    반복 코드는 네이티브로 최적화                    │
└──────────────────────┬──────────────────────────────┘
                       │
                       ▼
